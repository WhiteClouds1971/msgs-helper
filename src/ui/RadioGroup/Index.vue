<script setup>
   ...
   * 「主公（66.67%）」必然被截成省略号 —— 宁可多占一行，也不让文字换行或截断。
   */
...
<style scoped lang="less">
  ...
  /* ── 带补充说明的选项组（如身份 + 胜率）：整组换成两列网格 ──
     说明跟在名字后面同一行（「主公（66.67%）」），而一排四枚时单枚只有 70 来像素，
     整句必然被截成省略号 —— 那就别再挤一排：换成两列，单枚 150px 上下，整句放得下。
     只动带说明的这一组：不带说明的（对局那一行）照旧一行等分，两行控件看着才是一套。

     列的宽度 = minmax(max(9em, 两列平分), 1fr)：正常就是两列；窄到两列各自不足 9em
     （最长的一句「一号位（100.00%）」实测要 137px，留出余量取 144px）时自动收成一列 ——
     宁可一个按钮独占一行，也不把括号里的胜率截断。
     不写成媒体查询：看的是这块地方还剩多少宽，不是屏幕多宽（本页窄到 380px 上下收成一列）。
     减 1px 是为了让「刚好两列」的边界稳稳落在两列这边（不然整除误差会把它挤成一列） */
  .radio-field__options.is-hinted {
    display: grid;
    grid-template-columns: repeat(
      auto-fit,
      minmax(max(9em, calc((100% - var(--space-2)) / 2 - 1px)), 1fr)
    );
  }
