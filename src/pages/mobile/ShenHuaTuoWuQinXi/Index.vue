<script setup>
  ...
  const SKILLS = Object.freeze([
    ...
    },
    {
      ...
      name: '猿',
      description:
        '当你获得「猿」时，你选择一名其他角色，获得其装备区里的一张牌。',
    },
    {
      id: 'he',
      name: '鹤',
...
<template>
  <div class="shen-hua-tuo-wu-qin-xi">
    ...
    <section id="wqx-skills" class="wqx__skills">
      <header class="wqx__head">
        <h1 class="wqx__title">五禽戏</h1>
        <!-- id 归导览定位（#wqx-reset），样式归 @/ui/Button 的幽灵变体 -->
        <Button id="wqx-reset" variant="ghost" @click="handleReset">
          重置
        </Button>
      </header>

      <p class="wqx__hint">长按技能拖动排序 · 左滑移除 · 点「重置」还原</p>
      ...
      <ul ref="boardRef" class="skill-board">
        <li
        ...
        >
          <span class="skill-board__node" aria-hidden="true" />

          <div class="skill-board__viewport">
            <span class="skill-board__bed" aria-hidden="true">移除</span>

            <SkillCard class="skill-board__card" :skill="skill" />
          </div>
        </li>

        <li v-if="!skills.length" class="skill-board__empty">
          技能已全部移除 · 点上方「重置」找回五禽
        </li>
      </ul>
    </section>
    ...
    <div id="wqx-banner" ref="bannerBoxRef">
      <ImageFigure :src="bannerUrl" alt="五禽戏大旗" />
    </div>
  </div>
</template>
